...
public class ImgEditor {
            ...
            if(srcWidth > 0 && srcHeight > 0){
	            ...
	            if(!file2.getParentFile().exists()){
	                ...
	                if(!file2.getParentFile().mkdirs()){
	                    // 复制文件失败：创建目标文件所在目录失败
	                	throw new RuntimeException("失败：创建目标文件所在目录失败 ..........");
	                }
	            }
                // 输出为文件
                ImageIO.write(tag,"JPEG",file2);
            }  
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
  ...
  //获得文件名字  
    public String getFileName(MultipartFile file, HttpServletRequest request,HttpSession session){  
        ...
        String userName = "SuperAmdmin";//SecurityContextHolder.getContext().getAuthentication().getName();  
